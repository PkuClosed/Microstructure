...
for iMask in range(len(allMaskNames)):
    ...
    smt = MultiCompartmentSphericalMeanModel(models=[zeppelin])
    print smt.parameter_names
    
    smt.set_fractional_parameter('G2Zeppelin_1_lambda_perp', 'G2Zeppelin_1_lambda_par')
    print smt.parameter_names
    
    smt_fit = smt.fit(acq_scheme, dwi, Ns = 30, mask = mask)

    fitted_parameters = smt_fit.fitted_parameters
